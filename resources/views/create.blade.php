    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('create-pet-form').addEventListener('submit', function(event) {
                ...
                if (!validateForm(name, type_id, address, allowedTypes)) {
                    return;
                }
            ...
            function validateForm(name, type, address, allowedTypes) {
                type = parseInt(type);
                if (!allowedTypes.includes(type)) {
                    alert('Type is niet geldig');
                    return false;
                }
                if (name.length > 255) {
                    alert('Naam mag maximaal 255 karakters bevatten');
                    return false;
                }
                if (address.length > 255) {
                    alert('Adres mag maximaal 255 karakters bevatten');
                    return false;
                }
                return true;
            }
